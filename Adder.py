...
class addmembersMod(loader.Module):
    ...
    async def adduserscmd(self, event):
        if len(event.text.split()) == 2:
            ...
            await event.edit(f"<code>{len(user)} пользователей будет приглашено из чата {event.to_id.channel_id} в чат/канал {idschannelgroup}</code>")
            for u in user:
                try:
                    try:
                        if u.bot == False:
            ...
            await event.edit(f"<code>Куда приглашать будем?</code>")
    
    async def kickallcmd(self, event):
        user = [i async for i in event.client.iter_participants(event.to_id.channel_id)]
        await event.edit(f"<code>{len(user)} пользователей будет кикнуто из чата {event.to_id.channel_id}</code>")
        for u in user:
            try:
                try:
                    if u.is_self != True:
